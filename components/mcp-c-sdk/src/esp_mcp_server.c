...
static esp_err_t esp_mcp_server_add_mbuf(esp_mcp_server_t *server, uint16_t id, uint8_t *outbuf, uint16_t outlen)
{
    if (server->mbuf.outbuf) {
        cJSON_free(server->mbuf.outbuf);
        server->mbuf.outbuf = NULL;
        server->mbuf.outlen = 0;
    }

    server->mbuf.outbuf = outbuf;
    server->mbuf.outlen = outlen;
...
static esp_err_t esp_mcp_server_reply_result(esp_mcp_server_t *server, int id, const char *result)
{
    ...
    if (payload_str) {
        esp_mcp_server_add_mbuf(server, id, (uint8_t*)payload_str, strlen(payload_str));
    }
...
static esp_err_t esp_mcp_server_reply_error(esp_mcp_server_t *server, int error_code, const char *message, int request_id)
{
    ...
    char *payload_str = cJSON_PrintUnformatted(payload);
    if (payload_str) {
        esp_mcp_server_add_mbuf(server, request_id, (uint8_t*)payload_str, strlen(payload_str));
    }
    cJSON_Delete(payload);
...
static esp_err_t esp_mcp_server_tools_foreach_cb(esp_mcp_tool_t *tool, void *arg)
{
...
static esp_err_t esp_mcp_server_handle_tools_list(esp_mcp_server_t *server, const cJSON *params, int id)
{
    ...
    esp_mcp_tool_list_foreach(server->tools, esp_mcp_server_tools_foreach_cb, &ctx);
    ...
    if (payload_str) {
        bool list_not_empty = !esp_mcp_tool_list_is_empty(server->tools);
        if (strlen(payload_str) == 12 && list_not_empty) {
            ...
            if (error_msg) {
                ...
                esp_mcp_server_reply_error(server, MCP_ERROR_CODE_FAILED_TO_ADD_TOOL, error_msg, id);
                free(error_msg);
            } else {
                esp_mcp_server_reply_error(server, MCP_ERROR_CODE_FAILED_TO_ADD_TOOL, MCP_ERROR_MESSAGE_FAILED_TO_ADD_TOOL, id);
            }
            ret = ESP_ERR_NO_MEM;
        } else {
...
static esp_err_t esp_mcp_server_tool_args_cb(const esp_mcp_property_t *property, void *arg)
{
...
static esp_err_t esp_mcp_server_do_tool_call(esp_mcp_server_t *server, int id, const char *tool_name, const cJSON *tool_arguments, int stack_size)
{
    ...
    if (!tool) {
        ...
        if (error_msg) {
            ...
            esp_mcp_server_reply_error(server, MCP_ERROR_CODE_INVALID_PARAMS, error_msg, id);
            free(error_msg);
        } else {
            esp_mcp_server_reply_error(server, MCP_ERROR_CODE_INVALID_PARAMS, MCP_ERROR_MESSAGE_INVALID_PARAMS, id);
        }
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (arguments) {
        if (tool_arguments && cJSON_IsObject(tool_arguments)) {
            esp_mcp_property_foreach_ctx_t args_ctx = {
                .list = arguments,
                .callback = esp_mcp_server_tool_args_cb,
        ...
        if (result) {
            ...
            esp_mcp_server_reply_error(server, MCP_ERROR_CODE_TOOL_CALL_FAILED, MCP_ERROR_MESSAGE_TOOL_CALL_FAILED, id);
        }

        esp_mcp_property_list_destroy(arguments);
    } else {
        esp_mcp_server_reply_error(server, MCP_ERROR_CODE_FAILED_TO_CREATE_ARGUMENTS, MCP_ERROR_MESSAGE_FAILED_TO_CREATE_ARGUMENTS, id);
    }
...
static esp_err_t esp_mcp_server_handle_tools_call(esp_mcp_server_t *server, const cJSON *params, int id)
{
    ...
    if (!cJSON_IsObject(params)) {
        ...
        esp_mcp_server_reply_error(server, MCP_ERROR_CODE_INVALID_REQUEST, MCP_ERROR_MESSAGE_INVALID_REQUEST, id);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t esp_mcp_server_get_mbuf(esp_mcp_server_t *server, uint8_t **outbuf, uint16_t *outlen)
{
    ...
    *outbuf = server->mbuf.outbuf;
    *outlen = server->mbuf.outlen;
...
esp_err_t esp_mcp_server_add_tool(esp_mcp_server_t *server, esp_mcp_tool_t *tool)
{
    ...
    return esp_mcp_tool_list_add_tool(server->tools, tool);
}
...
esp_err_t esp_mcp_server_remove_tool(esp_mcp_server_t *server, esp_mcp_tool_t *tool)
{
    ...
    return esp_mcp_tool_list_remove_tool(server->tools, tool);
}

static esp_err_t esp_mcp_server_parse_json_message(esp_mcp_server_t *server, const cJSON *json)
{
    ESP_RETURN_ON_FALSE(server, ESP_ERR_INVALID_ARG, TAG, "Invalid server");
    ...
    if (version == NULL || !cJSON_IsString(version) || strcmp(version->valuestring, "2.0") != 0) {
        ...
        esp_mcp_server_reply_error(server, MCP_ERROR_CODE_INVALID_REQUEST, MCP_ERROR_MESSAGE_INVALID_REQUEST, 0);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (!strcmp(method_str, "initialize")) {
        ...
        if (error_msg) {
            ...
            esp_mcp_server_reply_error(server, MCP_ERROR_CODE_METHOD_NOT_FOUND, error_msg, id_int);
            free(error_msg);
        } else {
            esp_mcp_server_reply_error(server, MCP_ERROR_CODE_METHOD_NOT_FOUND, MCP_ERROR_MESSAGE_METHOD_NOT_FOUND, id_int);
        }
    }
...
esp_err_t esp_mcp_server_parse_message(esp_mcp_server_t *server, const char *message)
{
    ...
    if (!json) {
        ...
        esp_mcp_server_reply_error(server, MCP_ERROR_CODE_PARSE_ERROR, MCP_ERROR_MESSAGE_PARSE_ERROR, 0);
        return ESP_ERR_INVALID_ARG;
    }
