...
static void thread_test_task(void *pvParameters)
{
    ...
    esp_mcp_server_t *server = params->server;
    ...
    for (int i = 0; i < THREAD_TEST_ITERATIONS; i++) {
        ...
        esp_err_t ret = esp_mcp_server_add_tool(server, tool);
        ...
        if (ret == ESP_OK) {
            ret = esp_mcp_server_remove_tool(server, tool);
            TEST_ASSERT_EQUAL(ESP_OK, ret);
        }
...
TEST_CASE("test_server_create_destroy", "[server]")
{
    esp_mcp_server_t *server = NULL;

    // Test create
    esp_err_t ret = esp_mcp_server_create(&server);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
    TEST_ASSERT_NOT_NULL(server);

    // Test destroy
    ret = esp_mcp_server_destroy(server);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
}

TEST_CASE("test_server_create_null", "[server]")
{
    esp_err_t ret = esp_mcp_server_create(NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
}

TEST_CASE("test_server_destroy_null", "[server]")
{
    esp_err_t ret = esp_mcp_server_destroy(NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
}
...
TEST_CASE("test_server_add_remove_tool", "[server][tool]")
{
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));
    ...
    ret = esp_mcp_server_add_tool(server, tool2);
    ...
    ESP_ERROR_CHECK(esp_mcp_server_destroy(server));
}

TEST_CASE("test_server_add_tool_null", "[server][tool]")
{
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));

    esp_err_t ret = esp_mcp_server_add_tool(server, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    esp_mcp_tool_t *tool = esp_mcp_tool_create("test_tool", "Test tool", test_tool_callback_bool);
    ret = esp_mcp_server_add_tool(NULL, tool);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_server_remove_tool_null", "[server][tool]")
{
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));

    esp_err_t ret = esp_mcp_server_remove_tool(server, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    ret = esp_mcp_server_remove_tool(NULL, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_server_thread_safety", "[server][thread_safety]")
{
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));
...
TEST_CASE("test_memory_leak_full_workflow", "[memory]")
{
    for (int i = 0; i < 5; i++) {
        ...
        ESP_ERROR_CHECK(esp_mcp_server_add_tool(server, tool));
        ESP_ERROR_CHECK(esp_mcp_server_remove_tool(server, tool));
...
typedef struct {
    esp_mcp_handle_t handle;
    bool started;
    void *config;
} mock_transport_item_t;

static esp_err_t mock_transport_init(esp_mcp_handle_t handle, esp_mcp_transport_handle_t *transport_handle)
{
...
TEST_CASE("test_mcp_init_deinit", "[mcp]")
{
    ...
    esp_mcp_config_t config = {
        .transport = mock_transport,
        .config = &test_config,
        .instance = server
    };

    esp_mcp_handle_t handle = 0;
    esp_err_t ret = esp_mcp_init(config, &handle);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
    TEST_ASSERT_NOT_EQUAL(0, handle);

    ret = esp_mcp_deinit(handle);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
...
TEST_CASE("test_mcp_init_null_handle", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_init(config, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
{
    esp_err_t ret = esp_mcp_deinit(0);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
}
...
TEST_CASE("test_mcp_start_stop", "[mcp]")
{
    ...
    esp_mcp_handle_t handle = 0;
    ESP_ERROR_CHECK(esp_mcp_init(config, &handle));

    // Start transport
    esp_err_t ret = esp_mcp_start(handle);
    TEST_ASSERT_EQUAL(ESP_OK, ret);

    // Stop transport
    ret = esp_mcp_stop(handle);
    TEST_ASSERT_EQUAL(ESP_OK, ret);

    // Deinit (must stop before deinit)
    ESP_ERROR_CHECK(esp_mcp_deinit(handle));
    ESP_ERROR_CHECK(esp_mcp_server_destroy(server));
}

TEST_CASE("test_mcp_start_invalid_handle", "[mcp]")
{
    esp_err_t ret = esp_mcp_start(0);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
}

TEST_CASE("test_mcp_stop_invalid_handle", "[mcp]")
{
    esp_err_t ret = esp_mcp_stop(0);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
}
...
TEST_CASE("test_mcp_register_unregister_endpoint", "[mcp]")
{
    ...
    // Register endpoint
    esp_err_t ret = esp_mcp_register_endpoint(handle, "test_endpoint", NULL);
    TEST_ASSERT_EQUAL(ESP_OK, ret);

    // Try to register same endpoint again (should fail)
    ret = esp_mcp_register_endpoint(handle, "test_endpoint", NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE, ret);

    // Unregister endpoint
    ret = esp_mcp_unregister_endpoint(handle, "test_endpoint");
    TEST_ASSERT_EQUAL(ESP_OK, ret);
...
TEST_CASE("test_mcp_register_endpoint_null", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_register_endpoint(handle, NULL, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    ret = esp_mcp_register_endpoint(0, "test", NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_mcp_unregister_endpoint_null", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_unregister_endpoint(handle, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    ret = esp_mcp_unregister_endpoint(0, "test");
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_mcp_req_handle", "[mcp]")
{
    ...
    // Register endpoint
    ESP_ERROR_CHECK(esp_mcp_register_endpoint(handle, "mcp_server", server));
    ...
    esp_err_t ret = esp_mcp_req_handle(handle, "mcp_server", (const uint8_t *)init_msg, strlen(init_msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
    ...
    if (outbuf) {
        esp_mcp_req_destroy_response(handle, outbuf);
    }
...
TEST_CASE("test_mcp_req_handle_invalid_endpoint", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_req_handle(handle, "nonexistent", (const uint8_t *)msg, strlen(msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_ERR_NOT_FOUND, ret);
...
TEST_CASE("test_mcp_req_handle_null_args", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_req_handle(0, "test", (const uint8_t *)msg, strlen(msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    ret = esp_mcp_req_handle(handle, NULL, (const uint8_t *)msg, strlen(msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_mcp_req_destroy_response", "[mcp]")
{
    ...
    ESP_ERROR_CHECK(esp_mcp_req_handle(handle, "mcp_server", (const uint8_t *)init_msg, strlen(init_msg), &outbuf, &outlen));
    TEST_ASSERT_NOT_NULL(outbuf);

    // Destroy response
    esp_err_t ret = esp_mcp_req_destroy_response(handle, outbuf);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
...
TEST_CASE("test_mcp_req_destroy_response_null", "[mcp]")
{
    ...
    esp_err_t ret = esp_mcp_req_destroy_response(handle, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);

    ret = esp_mcp_req_destroy_response(0, NULL);
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, ret);
...
TEST_CASE("test_mcp_full_workflow_with_server", "[mcp][server][integration]")
{
    // Create server
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));
    ...
    // Start transport
    ESP_ERROR_CHECK(esp_mcp_start(handle));
    ...
    esp_err_t ret = esp_mcp_req_handle(handle, "mcp_server", (const uint8_t *)tools_list_msg, strlen(tools_list_msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
    TEST_ASSERT_NOT_NULL(outbuf);
    TEST_ASSERT_GREATER_THAN(0, outlen);
    ...
    ESP_ERROR_CHECK(esp_mcp_stop(handle));
    ESP_ERROR_CHECK(esp_mcp_deinit(handle));
    ESP_ERROR_CHECK(esp_mcp_server_destroy(server));
}

TEST_CASE("test_mcp_tools_call_integration", "[mcp][server][tool][integration]")
{
    // Create server
    esp_mcp_server_t *server = NULL;
    ESP_ERROR_CHECK(esp_mcp_server_create(&server));
    ...
    esp_err_t ret = esp_mcp_req_handle(handle, "mcp_server", (const uint8_t *)tools_call_msg, strlen(tools_call_msg), &outbuf, &outlen);
    TEST_ASSERT_EQUAL(ESP_OK, ret);
    TEST_ASSERT_NOT_NULL(outbuf);
    TEST_ASSERT_GREATER_THAN(0, outlen);
