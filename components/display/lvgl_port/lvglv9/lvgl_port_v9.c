...
static const char *TAG = "lv_port_v9";
...
static lv_display_t  *display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    lv_color_format_t lvgl_color_format = lv_display_get_color_format(display);
    lvgl_color_format_bytes = lv_color_format_get_size(lvgl_color_format);
    ...
    buf1 = heap_caps_malloc(buffer_size * lvgl_color_format_bytes, (MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
    assert(buf1);
    buf2 = NULL;

    /* Request triple frame buffers from panel driver */
    ESP_ERROR_CHECK(lvgl_get_lcd_frame_buffer(panel_handle, 3, &fb0, &fb1, &fb2));
    front_fb = fb0;
    back_fb  = fb1;
    spare_fb = fb2;
    fb_size = LVGL_PORT_H_RES * LVGL_PORT_V_RES * lvgl_color_format_bytes;
    ...
    // Normally, for RGB LCD, we just use one buffer for LVGL rendering
    buffer_size = LVGL_PORT_H_RES * LVGL_PORT_BUFFER_HEIGHT;
    buf1 = heap_caps_malloc(buffer_size * lvgl_color_format_bytes, LVGL_PORT_BUFFER_MALLOC_CAPS);
    assert(buf1);
    ESP_LOGI(TAG, "LVGL buffer size: %dKB", buffer_size * lvgl_color_format_bytes / 1024);
#endif /* LVGL_PORT_AVOID_TEAR_ENABLE */

    lv_display_set_buffers(
        display, buf1, buf2, buffer_size * lvgl_color_format_bytes,
