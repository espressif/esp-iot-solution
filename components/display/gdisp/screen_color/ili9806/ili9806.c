...
lcd_driver_fun_t lcd_ili9806_default_driver = {
...
esp_err_t lcd_ili9806_init(const lcd_config_t *lcd_conf)
{
...
{
    memset(&g_lcd_handle, 0, sizeof(ili9806_dev_t));
    return LCD_IFACE_DEINIT(true);
}


esp_err_t lcd_ili9806_set_rotation(lcd_dir_t dir)
{
    esp_err_t ret;
    uint8_t reg_data = 0;
    reg_data &= ~MADCTL_RGB;
...
esp_err_t lcd_ili9806_get_info(lcd_info_t *info)
{
