...
static ili9486_dev_t g_lcd_handle;

lcd_driver_fun_t lcd_ili9486_default_driver = {
...
esp_err_t lcd_ili9486_init(const lcd_config_t *lcd_conf)
{
    ...
    LCD_CHECK(ESP_OK == ret, "i2s driver initial failed", ESP_FAIL);
...
{
    memset(&g_lcd_handle, 0, sizeof(ili9486_dev_t));
    return LCD_IFACE_DEINIT(true);
}

esp_err_t lcd_ili9486_set_rotation(lcd_dir_t dir)
{
    esp_err_t ret;
...
esp_err_t lcd_ili9486_get_info(lcd_info_t *info)
{
    ...
    info->name = LCD_NAME;
    info->color_type = LCD_COLOR_TYPE_65K;
    return ESP_OK;
}
