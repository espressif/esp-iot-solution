...
lcd_driver_fun_t lcd_ssd1351_default_driver = {
...
esp_err_t lcd_ssd1351_init(const lcd_config_t *lcd_conf)
{
    ...
    LCD_CHECK(ESP_OK == ret, "SPI driver initialization failed", ESP_FAIL);
...
esp_err_t lcd_ssd1351_set_rotation(lcd_dir_t dir)
{
    esp_err_t ret;
    if (LCD_DIR_MAX < dir) {
        dir >>= 5;
...
esp_err_t lcd_ssd1351_get_info(lcd_info_t *info)
{
