...
#define LCD_SSD1307_CHECK(a, str, ret)  if(!(a)) {                               \
...
typedef struct {
    ...
    uint8_t gram[SSD1307_PAGES][SSD1307_COLUMNS];
#endif
    uint16_t width;
    uint16_t height;
} ssd1307_dev_t;
...
static esp_err_t lcd_ssd1307_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1);
static esp_err_t lcd_ssd1307_write_ram_data(uint16_t color);

lcd_driver_fun_t lcd_ssd1307_default_driver = {
...
esp_err_t lcd_ssd1307_init(const lcd_config_t *lcd_conf)
{
    LCD_SSD1307_CHECK(lcd_conf->width <= SSD1307_COLUMNS, "Width greater than maximum", ESP_ERR_INVALID_ARG);
    LCD_SSD1307_CHECK(lcd_conf->height <= SSD1307_HEIGHT, "Height greater than maximum", ESP_ERR_INVALID_ARG);
    ...
    LCD_SSD1307_CHECK(ESP_OK == ret, "i2c lcd driver initial failed", ESP_FAIL);
...
esp_err_t lcd_ssd1307_get_info(lcd_info_t *info)
{
...
esp_err_t lcd_ssd1307_set_rotate(lcd_dir_t dir)
{
    LCD_SSD1307_CHECK(dir < 4, "Unsupport rotate direction", ESP_ERR_INVALID_ARG);
    ...
    LCD_SSD1307_CHECK(ESP_OK == ret, "Set screen rotate failed", ESP_FAIL);
    return ESP_OK;
}

static esp_err_t lcd_ssd1307_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ESP_LOGW(TAG, "lcd ssd1307 unsupport set window");
...
esp_err_t lcd_ssd1307_draw_pixel(uint16_t x, uint16_t y, uint16_t chPoint)
{
    LCD_SSD1307_CHECK((x < g_lcd_handle.width) && (y < g_lcd_handle.height), "The set coordinates exceed the screen size", ESP_ERR_INVALID_ARG)
#if CONFIG_MONO_SCREEN_BUFFER
...
{
    LCD_SSD1307_CHECK((x + w <= g_lcd_handle.width) && (y + h <= g_lcd_handle.height), "The set coordinates exceed the screen size", ESP_FAIL);
    esp_err_t ret = ESP_OK;
    ...
    for (j = 0; j < h; j++) {
        for (i = 0; i < w; i++) {
            ...
            if (bits & (1 << (j%8))) {
                ret |= lcd_ssd1307_draw_pixel(x + i, y + j, 1);
            }else{
                ret |= lcd_ssd1307_draw_pixel(x + i, y + j, 0);
            }
        }
    }
#else
    uint8_t row1 = y >> 3;
    uint8_t row2 = (h + y + 7) / 8;printf("[%d][%d]\n", row1, row2);
    ...
    ret |= LCD_WRITE(p, SSD1307_COLUMNS * (row2 - row1));
#endif
    LCD_SSD1307_CHECK(ESP_OK == ret, "Draw bitmap failed", ESP_FAIL);
    return ESP_OK;
}

#if CONFIG_MONO_SCREEN_BUFFER
esp_err_t lcd_ssd1307_refresh_gram(void)
{
    esp_err_t ret = ESP_OK;
    ...
    ret |= LCD_WRITE_CMD(SSD1307_CMD_SET_COLUMN_RANGE);
    ret |= LCD_WRITE_CMD(0);
    ret |= LCD_WRITE_CMD(SSD1307_COLUMNS - 1);
    ret |= LCD_WRITE_CMD(SSD1307_CMD_SET_PAGE_RANGE);
    ret |= LCD_WRITE_CMD(0);
    ret |= LCD_WRITE_CMD(SSD1307_PAGES - 1);
    ret |= LCD_WRITE(&g_lcd_handle.gram[0][0], SSD1307_COLUMNS * SSD1307_PAGES);
    LCD_SSD1307_CHECK(ESP_OK == ret, "refresh GRAM failed", ESP_FAIL);
    return ESP_OK;
}
#endif

esp_err_t lcd_ssd1307_clear_screen(void)
{
    uint8_t i, j;
    esp_err_t ret;

    for (i = 0; i < SSD1307_PAGES; i++) {
        ret = LCD_WRITE_CMD(SSD1307_CMD_SET_PAGE_ADDR + i);
        ret |= LCD_WRITE_CMD(SSD1307_LOWER_ADDRESS);
        ret |= LCD_WRITE_CMD(SSD1307_HIGHER_ADDRESS);
#if CONFIG_MONO_SCREEN_BUFFER
        for (j = 0; j < SSD1307_COLUMNS; j++) {
            g_lcd_handle.gram[i][j] = 0;
        }
#endif
        ret |= LCD_WRITE(g_lcd_handle.gram[i], SSD1307_COLUMNS);
        LCD_SSD1307_CHECK(ESP_OK == ret, "Write data failed", ESP_FAIL);
    }
    return ESP_OK;
}
...
esp_err_t lcd_ssd1307_display_on(void)
{
    ...
    LCD_SSD1307_CHECK(ESP_OK == ret, "Set display ON failed", ESP_FAIL);
    return ESP_OK;
}
...
esp_err_t lcd_ssd1307_display_off(void)
{
    ...
    LCD_SSD1307_CHECK(ESP_OK == ret, "Set display OFF failed", ESP_FAIL);
    return ESP_OK;
}
...
esp_err_t lcd_ssd1307_set_contrast(uint8_t contrast)
{
    ...
    LCD_SSD1307_CHECK(ESP_OK == ret, "Set contrast failed", ESP_FAIL);
    return ESP_OK;
}

esp_err_t lcd_ssd1307_start_horizontal_scroll(uint8_t dir, uint8_t start, uint8_t stop, uint8_t interval)
{
    LCD_SSD1307_CHECK(start < SSD1307_PAGES, "Start page address invalid", ESP_ERR_INVALID_ARG);
    LCD_SSD1307_CHECK(stop < SSD1307_PAGES, "End page address invalid", ESP_ERR_INVALID_ARG);
    LCD_SSD1307_CHECK(interval < 8, "Time interval invalid", ESP_ERR_INVALID_ARG);
