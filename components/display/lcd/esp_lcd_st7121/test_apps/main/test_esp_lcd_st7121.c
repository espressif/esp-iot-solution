...
static void test_init_lcd(void)
{
...
#if ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(6, 0, 0)
    esp_lcd_dpi_panel_config_t dpi_config = ST7121_1280_720_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
#else
    esp_lcd_dpi_panel_config_t dpi_config = ST7121_1280_720_PANEL_60HZ_DPI_CONFIG_CF(TEST_MIPI_DPI_PX_FORMAT);
#endif
    ...
    TEST_ESP_OK(esp_lcd_new_panel_st7121(mipi_dbi_io, &panel_config, &panel_handle));
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0)
...
TEST_CASE("test st7121 to draw pattern with MIPI interface", "[st7121][draw_pattern]")
{
...
TEST_CASE("test st7121 to draw color bar with MIPI interface", "[st7121][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test st7121 to rotate with MIPI interface", "[st7121][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
void app_main(void)
{
    ...
    * |____/ |_|  /_/  |_|_____|_|
    */
    printf("     ___ _____ _____ _ ____  _  \r\n");
