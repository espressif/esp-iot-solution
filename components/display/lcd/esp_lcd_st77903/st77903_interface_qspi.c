...
static bool load_trans_pool(st77903_qspi_panel_t *panel, spi_multi_transaction_t *seg_trans, bool in_isr);
...
static esp_err_t st77903_qspi_alloc_buffers(st77903_qspi_panel_t *panel)
{
    ...
    ESP_LOGD(TAG, "segment_gap_clock_len: %ld", trans_pool_temp.base.sct_gap_len);
    for (int i = 0; i < panel->trans_pool_num; i++) {
...
static esp_err_t panel_st77903_qspi_draw_bitmap(esp_lcd_panel_t *handle, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    ESP_RETURN_ON_FALSE((x_start < x_end) && (x_end - x_start <= panel->hor_res), ESP_ERR_INVALID_ARG, TAG, "Invalid x(%d, %d)", x_start, x_end);
    ESP_RETURN_ON_FALSE((y_start < y_end) && (y_end - y_start <= panel->ver_res), ESP_ERR_INVALID_ARG, TAG, "Invalid y(%d, %d)", y_start, y_end);
...
IRAM_ATTR static bool load_trans_pool(st77903_qspi_panel_t *panel, spi_multi_transaction_t *seg_trans, bool in_isr)
{
