...
static const char *TAG = "st77926_test";
...
TEST_CASE("test st77926 to draw color bar with QSPI interface", "[st77926][qspi]")
{
    ...
    const esp_lcd_panel_io_spi_config_t io_config = ST77926_PANEL_IO_QSPI_CONFIG(TEST_PIN_NUM_LCD_CS, test_notify_refresh_ready, NULL);
    TEST_ESP_OK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)TEST_LCD_HOST, &io_config, &io_handle));

    ESP_LOGI(TAG, "Install LCD driver of st77926");
    ...
    TEST_ESP_OK(esp_lcd_new_panel_st77926(io_handle, &panel_config, &panel_handle));
