...
#define TEST_MIPI_DSI_LANE_NUM          (1)
...
static char *TAG = "st77922_mipi_test";
...
static void test_init_lcd(void)
{
    ...
    ESP_LOGI(TAG, "Install LCD driver of st77922_mipi");
    esp_lcd_dpi_panel_config_t dpi_config = ST77922_MIPI_480_480_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_st77922(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test st77922_mipi to draw pattern with MIPI interface", "[st77922_mipi][draw_pattern]")
{
...
TEST_CASE("test st77922_mipi to draw color bar with MIPI interface", "[st77922_mipi][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test st77922_mipi to rotate with MIPI interface", "[st77922_mipi][rotate]")
{
