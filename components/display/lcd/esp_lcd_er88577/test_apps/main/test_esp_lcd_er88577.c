...
static void test_init_lcd(void)
{
    ...
    esp_lcd_dsi_bus_config_t bus_config = ER88577_PANEL_BUS_DSI_2CH_CONFIG();
    TEST_ESP_OK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus));

    ESP_LOGI(TAG, "Install panel IO");
    esp_lcd_dbi_io_config_t dbi_config = ER88577_PANEL_IO_DBI_CONFIG();
    TEST_ESP_OK(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &mipi_dbi_io));

    ESP_LOGI(TAG, "Install LCD driver of er88577");
    esp_lcd_dpi_panel_config_t dpi_config = ER88577_800_1280_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_er88577(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test er88577 to draw pattern with MIPI interface", "[er88577][draw_pattern]")
{
...
TEST_CASE("test er88577 to draw color bar with MIPI interface", "[er88577][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test er88577 to rotate with MIPI interface", "[er88577][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
{
    /**
     *  _____ ____  ___   ___ ____ _____ _____
     ...
     *
     */
    printf("  _____ ____  ___   ___ ____ _____ _____  \r\n");
    printf(" | ____|  _ \\( _ ) ( _ ) ___|___  |___  | \r\n");
    printf(" |  _| | |_) / _ \\ / _ \\___ \\  / /   / /  \r\n");
    printf(" | |___|  _ < (_) | (_) |__) |/ /   / /   \r\n");
    printf(" |_____|_| \\_\\___/ \\___/____//_/   /_/    \r\n");
    printf("                                          \r\n");
