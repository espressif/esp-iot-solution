...
#define TEST_LCD_H_RES              (454)
#define TEST_LCD_V_RES              (454)
...
TEST_CASE("test sh8601 to draw color bar with SPI interface", "[sh8601][spi]")
{
    ...
    const spi_bus_config_t buscfg = SH8601_PANEL_BUS_SPI_CONFIG(TEST_PIN_NUM_LCD_PCLK, TEST_PIN_NUM_LCD_DATA0);
...
{
    ESP_LOGI(TAG, "Initialize SPI bus");
    const spi_bus_config_t buscfg = SH8601_PANEL_BUS_QSPI_CONFIG(TEST_PIN_NUM_LCD_PCLK, TEST_PIN_NUM_LCD_DATA0,
