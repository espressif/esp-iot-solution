...
static const char *TAG = "gc9503_mipi_test";
...
static void test_init_lcd(void)
{
    ...
    ESP_LOGI(TAG, "Install LCD driver of gc9503_mipi");
    esp_lcd_dpi_panel_config_t dpi_config = GC9503_MIPI_480_800_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_gc9503_mipi(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test gc9503_mipi to draw pattern with MIPI interface", "[gc9503_mipi][draw_pattern]")
{
...
TEST_CASE("test gc9503_mipi to draw color bar with MIPI interface", "[gc9503_mipi][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test gc9503_mipi to rotate with MIPI interface", "[gc9503_mipi][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
void app_main(void)
{
     ...
     */
    printf("  ____  ____ ___  ____   ___ _____ \r\n");
