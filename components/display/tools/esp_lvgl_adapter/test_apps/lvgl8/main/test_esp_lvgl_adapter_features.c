...
/* v8 shares one LVGL instance; first use of FreeType / mmap / lv_fs
 * triggers one-time lazy allocations (~2.5 KB) that persist. */
...
/* LVGL v8 has no lv_deinit(), so repeated init/deinit cycles are unsafe.
 * Initialize once in app_main and share across all tests. */
void setUp(void)
{
...
TEST_CASE("adapter freetype font", "[adapter][freetype]")
{
    ...
    /* LVGL v8 FreeType doesn't support lv_fs; use mmap pointer directly (zero-copy) */
