...
typedef struct {
    ...
    blink_step_t **blink_lists;     /*!< user defined led blink lists */
...
led_indicator_handle_t led_indicator_create(int io_num, const led_indicator_config_t *config)
{
    ...
    LED_INDICATOR_CHECK(config->blink_lists != NULL, "led indicator blink lists can't be NULL", NULL);
    char timer_name[16] = {'\0'};
    snprintf(timer_name, sizeof(timer_name) - 1, "%s%02x", "led_tmr_", io_num);
    ...
    p_led_indicator->blink_lists = config->blink_lists;
    p_led_indicator->p_blink_steps = (int *)calloc(config->blink_list_num, sizeof(int));
    p_led_indicator->blink_list_num = config->blink_list_num;
    ...
    for (size_t j = 0; j < config->blink_list_num; j++) {
        *(p_led_indicator->p_blink_steps + j) = LED_BLINK_STOP;
    }

    switch (p_led_indicator->mode) {
    case LED_GPIO_MODE: {      /**< blink with max brightness*/
        bool initted = _led_gpio_init(p_led_indicator->io_num);
        LED_INDICATOR_CHECK_GOTO(initted != false, "init led gpio failed", cleanup_all);
        p_led_indicator->h_timer = xTimerCreate(timer_name, (pdMS_TO_TICKS(100)), pdFALSE, (void *)p_led_indicator, _blink_list_runner);
        LED_INDICATOR_CHECK_GOTO(p_led_indicator->h_timer != NULL, "led timer create failed", cleanup_all);
    }
    break;
...
blink_step_t const **led_indicator_get_sample_lists(void)
{
    return led_indicator_blink_lists;
}

uint16_t led_indicator_get_sample_lists_num(void)
{
    return BLINK_LIST_NUM;
}
...
esp_err_t led_indicator_start(led_indicator_handle_t handle, led_indicator_blink_type_t blink_type)
{
    ...
    LED_INDICATOR_CHECK(p_led_indicator->blink_lists[blink_type] != NULL, "undefined blink_type", ESP_ERR_INVALID_ARG);
    xSemaphoreTake(p_led_indicator->mutex, portMAX_DELAY);
