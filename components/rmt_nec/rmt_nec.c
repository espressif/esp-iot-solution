...
{
	RMT_CHECK(ir_nec.channel < RMT_CHANNEL_MAX, RMT_CHANNEL_ERROR_STR, ESP_ERR_INVALID_ARG);
    rmt_config(&ir_nec);
    if(ir_nec.rmt_mode == RMT_MODE_TX){
    	rmt_driver_install(ir_nec.channel, 0, 0);
    }
    else if(ir_nec.rmt_mode == RMT_MODE_RX){
    	rmt_driver_install(ir_nec.channel, 1000, 0);
    }
    return ESP_OK;
}
    ...
    while(rb) {
        ...
        if(item) {
            ...
            while(1) {
                ...
                if(res > 0) {
                    offset += res + 1;
                    (*ir_recv_data)[0] = rmt_addr;
                    (*ir_recv_data)[1] = rmt_cmd;
