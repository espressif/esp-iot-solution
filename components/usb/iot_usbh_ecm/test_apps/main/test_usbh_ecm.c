...
TEST_CASE("usbh ecm device memory leak", "[iot_usbh_ecm][read-write][auto]")
{
    UPDATE_LEAK_THRESHOLD(-40);

    iot_eth_driver_t *ecm_eth_driver = NULL;
    ...
    TEST_ESP_OK(iot_eth_new_usb_ecm(&ecm_cfg, &ecm_eth_driver));

    iot_eth_config_t eth_cfg = {
        .driver = ecm_eth_driver,
        .stack_input = NULL,
    };
    ...
    ESP_LOGI(TAG, "ecm device started");

    vTaskDelay(10000 / portTICK_PERIOD_MS);

    ESP_LOGI(TAG, "ecm device stop");
