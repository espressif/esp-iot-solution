...
{
    SENSOR_CHECK(sensor != NULL && sensor_data != NULL, "pointer can't be NULL ", ESP_ERR_INVALID_STATE);
    sensor_imu_t *p_sensor = (sensor_imu_t *)(sensor);
    esp_err_t ret = p_sensor->impl->acquire_acce(&sensor_data->acce.x, &sensor_data->acce.y, &sensor_data->acce.z);
    sensor_data->event_id = SENSOR_ACCE_DATA_READY;
    return ret;
}
...
esp_err_t imu_acquire_gyro(sensor_imu_handle_t sensor, sensor_data_t *sensor_data)
{
    SENSOR_CHECK(sensor != NULL && sensor_data != NULL, "pointer can't be NULL ", ESP_ERR_INVALID_STATE);
    sensor_imu_t *p_sensor = (sensor_imu_t *)(sensor);
    esp_err_t ret = p_sensor->impl->acquire_gyro(&sensor_data->gyro.x, &sensor_data->gyro.y, &sensor_data->gyro.z);
    sensor_data->event_id = SENSOR_GYRO_DATA_READY;
    return ret;
}
...
esp_err_t imu_set_power(sensor_imu_handle_t sensor, sensor_power_mode_t power_mode)
{
    SENSOR_CHECK(sensor != NULL, "pointer can't be NULL ", ESP_ERR_INVALID_STATE);
    sensor_imu_t *p_sensor = (sensor_imu_t *)(sensor);
    esp_err_t ret;
    switch (power_mode)
    {
...
{
    SENSOR_CHECK(sensor != NULL && data_group != NULL, "pointer can't be NULL ", ESP_ERR_INVALID_STATE);
    sensor_imu_t *p_sensor = (sensor_imu_t *)(sensor);
    esp_err_t ret;
    ret = p_sensor->impl->acquire_gyro(&data_group->sensor_data[0].gyro.x, &data_group->sensor_data[0].gyro.y, &data_group->sensor_data[0].gyro.z);
    data_group->sensor_data[0].event_id = SENSOR_GYRO_DATA_READY;
    ret = p_sensor->impl->acquire_acce(&data_group->sensor_data[1].acce.x, &data_group->sensor_data[1].acce.y, &data_group->sensor_data[1].acce.z);
    data_group->sensor_data[1].event_id = SENSOR_ACCE_DATA_READY;
    data_group->number = 2;
    return ret;
}
