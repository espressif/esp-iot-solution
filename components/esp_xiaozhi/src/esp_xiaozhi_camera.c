...
static const char *TAG = "ESP_XIAOZHI_CAMERA";

struct esp_xiaozhi_camera_ctx {
    esp_xiaozhi_vision_t vision;
};

esp_err_t esp_xiaozhi_camera_create(const esp_xiaozhi_camera_config_t *config,
                                    esp_xiaozhi_camera_handle_t **out_handle)
{
    ESP_RETURN_ON_FALSE(out_handle != NULL, ESP_ERR_INVALID_ARG, TAG, "Invalid output handle");

    *out_handle = NULL;
    esp_xiaozhi_camera_handle_t *handle = calloc(1, sizeof(*handle));
...
esp_err_t esp_xiaozhi_camera_set_explain_url(esp_xiaozhi_camera_handle_t *handle,
                                             const char *url,
                                             const char *token)
{
    ESP_RETURN_ON_FALSE(handle != NULL, ESP_ERR_INVALID_ARG, TAG, "Invalid handle");
