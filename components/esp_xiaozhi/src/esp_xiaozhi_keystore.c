...
static const esp_xiaozhi_nvs_ops_t s_default_nvs_ops = {
    .nvs_open      = default_nvs_open,
    .nvs_close     = nvs_close,
    .nvs_commit    = nvs_commit,
    .nvs_get_str   = nvs_get_str,
    .nvs_set_str   = nvs_set_str,
    .nvs_get_i32   = nvs_get_i32,
    .nvs_set_i32   = nvs_set_i32,
    .nvs_erase_key = nvs_erase_key,
    .nvs_erase_all = nvs_erase_all,
};

static const esp_xiaozhi_nvs_ops_t *s_nvs_ops = NULL;

static const esp_xiaozhi_nvs_ops_t *get_nvs_ops(void)
{
...
static esp_err_t validate_nvs_ops(const esp_xiaozhi_nvs_ops_t *ops)
{
...
esp_err_t esp_xiaozhi_chat_keystore_init(esp_xiaozhi_chat_keystore_t *xiaozhi_chat_keystore, const char *name_space, bool read_write)
{
    ...
    const esp_xiaozhi_nvs_ops_t *ops = get_nvs_ops();
    ...
    memcpy(xiaozhi_chat_keystore->name_space, name_space, ns_len - 1);
                                               ...
                                               const char *default_value, char *out_value, uint16_t max_len)
{
    ...
    if (ret == ESP_ERR_NVS_NOT_FOUND) {
        if (default_value != NULL) {
            size_t copy_len = strnlen(default_value, max_len - 1);
