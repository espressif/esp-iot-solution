    ...
    m_horizontal_flip(horizontal_flip)
{
    ...
    memset(m_cam_fbs, 0, sizeof(cam_fb_t) * m_fb_count);
    video_init();
}
    ...
    esp_video_init_csi_config_t csi_config[] = {
        {
            .sccb_config = {
                ...
                .i2c_config = {
                    .port = static_cast<i2c_port_t>(CONFIG_EXAMPLE_MIPI_CSI_SCCB_I2C_PORT),
...
cam_fb_t *Camera::cam_fb_peek(bool back)
{
    ...
    xSemaphoreTake(m_mutex, portMAX_DELAY);
    cam_fb_t *fb = back ? m_buf_queue.back() : m_buf_queue.front();
    xSemaphoreGive(m_mutex);
    return fb;
}
