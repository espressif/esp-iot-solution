...
static void app_led_light_init(esp_weaver_node_t *node)
{
    esp_weaver_device_t *light_device = app_light_device_create(
                                            DEFAULT_POWER, DEFAULT_BRIGHTNESS, DEFAULT_HUE, DEFAULT_SATURATION);
    if (!light_device) {
        ESP_LOGE(TAG, "Could not create light device, aborting");
        vTaskDelay(5000 / portTICK_PERIOD_MS);
        abort();
    }

    esp_err_t err = esp_weaver_node_add_device(node, light_device);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add light device to node: %s, aborting", esp_err_to_name(err));
        vTaskDelay(5000 / portTICK_PERIOD_MS);
        abort();
    }
}
