...
static const uint8_t hidReportMap[] = {
    ...
    0x09, 0x02,  // Usage (Mouse)
    0xA1, 0x01,  // Collection (Application)
    0x85, HID_RPT_ID_ABS_MOUSE_IN,  // Report Id (5)
    0x09, 0x01,  //   Usage (Pointer)
    0xA1, 0x00,  //   Collection (Physical)
    0x05, 0x09,  //     Usage Page (Buttons)
    0x19, 0x01,  //     Usage Minimum (01) - Button 1
...
// HID Report Reference characteristic descriptor, absolute mouse input
static uint8_t hidReportRefAbsMouseIn[HID_REPORT_REF_LEN] =
{ HID_RPT_ID_ABS_MOUSE_IN, HID_REPORT_TYPE_INPUT };
...
static esp_gatts_attr_db_t hidd_le_gatt_db[HIDD_LE_IDX_NB] = {
    ...
    [HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_CHAR]   = {{ESP_GATT_AUTO_RSP}, {
            ESP_UUID_LEN_16, (uint8_t *) &character_declaration_uuid,
            ESP_GATT_PERM_READ,
            CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE,
            (uint8_t *) &char_prop_read_notify
    ...
    [HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_VAL]    = {{ESP_GATT_AUTO_RSP}, {
            ESP_UUID_LEN_16, (uint8_t *) &hid_report_uuid,
            ESP_GATT_PERM_READ,
            HIDD_LE_REPORT_MAX_LEN, 0,
            NULL
    ...
    [HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_CCC]    = {{ESP_GATT_AUTO_RSP}, {
            ESP_UUID_LEN_16, (uint8_t *) &character_client_config_uuid,
            (ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE),
            sizeof(uint16_t), 0,
            NULL
    ...
    [HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_REP_REF] = {{ESP_GATT_AUTO_RSP}, {
            ESP_UUID_LEN_16, (uint8_t *) &hid_report_ref_descr_uuid,
            ESP_GATT_PERM_READ,
...
static bool hid_cccd_notify_enabled(const esp_ble_gatts_cb_param_t *param)
{
    return param != NULL &&
           param->write.len == 2 &&
...
static void hid_notify_cccd_write(uint16_t conn_id,
                                  uint8_t report_id,
                                  uint8_t report_type,
                                  bool enabled)
{
    ...
    cb_param.report_cccd_write.conn_id = conn_id;
    cb_param.report_cccd_write.report_id = report_id;
    cb_param.report_cccd_write.report_type = report_type;
    cb_param.report_cccd_write.enabled = enabled;

    hidd_le_env.hidd_cb(ESP_HIDD_EVENT_BLE_REPORT_CCCD_WRITE_EVT, &cb_param);
}
                         ...
                         esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        const bool notify_enabled = hid_cccd_notify_enabled(param);

        if (handle == hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_MOUSE_IN_CCC]) {
            ESP_LOGI(HID_LE_PRF_TAG, "mouse input CCCD %s",
                     notify_enabled ? "enabled" : "disabled");
            hid_notify_cccd_write(param->write.conn_id,
                                  HID_RPT_ID_MOUSE_IN,
        ...
        if (handle == hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_CCC]) {
            ESP_LOGI(HID_LE_PRF_TAG, "absolute mouse input CCCD %s",
                     notify_enabled ? "enabled" : "disabled");
            hid_notify_cccd_write(param->write.conn_id,
                                  HID_RPT_ID_ABS_MOUSE_IN,
                                  HID_REPORT_TYPE_INPUT,
                                  notify_enabled);
            break;
        }

        if (handle == hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_KEY_IN_CCC]) {
            ESP_LOGI(HID_LE_PRF_TAG, "keyboard input CCCD %s",
                     notify_enabled ? "enabled" : "disabled");
            hid_notify_cccd_write(param->write.conn_id,
                                  HID_RPT_ID_KEY_IN,
                                  HID_REPORT_TYPE_INPUT,
        ...
        if (handle == hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_CC_IN_CCC]) {
            ESP_LOGI(HID_LE_PRF_TAG, "consumer control input CCCD %s",
                     notify_enabled ? "enabled" : "disabled");
            hid_notify_cccd_write(param->write.conn_id,
                                  HID_RPT_ID_CC_IN,
                                  HID_REPORT_TYPE_INPUT,
...
void hidd_set_attr_value(uint16_t handle, uint16_t val_len, const uint8_t *value)
{
    ...
    if (hidd_inst->att_tbl[HIDD_LE_IDX_HID_INFO_VAL] <= handle &&
            hidd_inst->att_tbl[HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_REP_REF] >= handle) {
...
static void hid_add_id_tbl(void)
{
    ...
    // Absolute mouse input report
    hid_rpt_map[8].id = hidReportRefAbsMouseIn[0];
    hid_rpt_map[8].type = hidReportRefAbsMouseIn[1];
    hid_rpt_map[8].handle = hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_VAL];
    hid_rpt_map[8].cccdHandle = hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_REPORT_ABS_MOUSE_IN_CCC];
