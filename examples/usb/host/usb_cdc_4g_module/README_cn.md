...
> 当主 modem 接口进入 ppp 网络状态后，Secondary AT Port 可以被用于 AT 交互。
...
* 开发板固定在三脚架上，垂直地面向上，距地高度大约 1.2m

* 开发板连接 Cat.1 模组，使用手机连接开发板热点

* 测试者站在距离三脚架 2m 处，使用测速软件测试上下行速度

### 测试结果
| 芯片型号 | 距离 |    测试结果Mbits/s    |
| :------: | :--: | :-------------------: |
| ESP32-S3  |  2m  | 下行：8.15 上行：4.12 |
...
* 两部手机，一部手机开启热点作为 server，使用 ESP32-S3 开发板连接该手机热点，并开启AP给另一台手机做 client

* 两名测试者站在距离三脚架 2m 处，使用WiFi魔盒测试软件中的 iperf 测试工具测试两部手机之间的上下行速度两次，取两次测试的平均值为测试结果

### 测试结果
| 芯片型号 | 距离 |    测试结果Mbits/s    |
| :------: | :--: | :-------------------: |
| ESP32-S3  |  2m  | 下行：4.48 上行：4.56 |
