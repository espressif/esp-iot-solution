...
该示例程序可实现 ESP 芯片作为 USB 主机驱动 4G 模组 PPP 拨号上网，并分享互联网给物联网设备或手持设备。同时配有路由器管理界面，可以在线进行路由器配置和查看已连接设备信息。
...
可在 `menuconfig` 的 `4G Modem Example Config → Enable USB PPP Download Speed Test` 中启用下载速度测试功能。启用此选项后，在成功获取 IP 地址后，程序会自动通过从远程服务器下载测试文件来测试下载速度。
