...
static jpeg_error_t esp_jpeg_decoder_one_picture(uint8_t *input_buf, int len, uint8_t *output_buf, int output_buf_len,
                                                 uint16_t width, uint16_t height)
{
    jpeg_error_t ret = JPEG_ERR_OK;
    if (!input_buf || len <= 0 || !output_buf || output_buf_len <= 0) {
    ...
    if (out_info.width != width || out_info.height != height) {
        ESP_LOGW(TAG, "Drop JPEG with unexpected size: %ux%u, expected %ux%u",
                 out_info.width, out_info.height, width, height);
        ret = JPEG_ERR_INVALID_PARAM;
        goto _exit;
    }

    int required_out_len = 0;
    ret = jpeg_dec_get_outbuf_len(jpeg_dec, &required_out_len);
    if (ret != JPEG_ERR_OK) {
        goto _exit;
    }
    if (required_out_len > output_buf_len) {
        ESP_LOGW(TAG, "JPEG output too large: %d > %d", required_out_len, output_buf_len);
        ret = JPEG_ERR_NO_MEM;
        goto _exit;
    }
...
void app_lcd_draw(uint8_t *buf, uint32_t len, uint16_t width, uint16_t height)
{
    ...
    jpeg_error_t ret = esp_jpeg_decoder_one_picture((uint8_t *)buf, len, jpeg_out_buf, EXAMPLE_LCD_BUF_LEN, width, height);
    if (ret != JPEG_ERR_OK) {
        ESP_LOGD(TAG, "JPEG decode failed: %d", ret);
        return;
    }

    esp_lcd_panel_draw_bitmap(display_handle, 0, 0, EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES, jpeg_out_buf);
...
#endif
    jpeg_out_buf = jpeg_calloc_align(EXAMPLE_LCD_BUF_LEN, 16);
    ESP_RETURN_ON_FALSE(jpeg_out_buf, ESP_ERR_NO_MEM, TAG, "Failed to allocate JPEG output buffer");
    return ESP_OK;
}
