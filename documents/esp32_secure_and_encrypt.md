...
* FLASH_CRYPT_CNT 的变化过程：
	...
	6. 重启设备，bootloader 发现 FLASH_CRYPT_CNT 的值是 0x0f（4 bit 1），于是重新加密 flash 数据，加密完成后 bootloader 将 FLASH_CRYPT_CNT 烧写为0x1f（5 bit 1），flash 加密正常使用
...
* 第一次 boot 时 secure boot 与 flash encrypt 的生效过程如下图所示，图中蓝色框是 secure boot 的步骤，绿色框是 flash encrypt 的步骤
	...
	<img src="./esp32_secure_encrypt/secure_encrypt_first_boot.png" width = "500" alt="touchpad_temp1" align=center />
	
* 后续 boot 时流程图如下，图中绿色框中的步骤会执行解密，解密是由硬件自动完成的

	<img src="./esp32_secure_encrypt/secure_encrypt_subsequent_boot.png" width = "500" alt="touchpad_temp1" align=center />
	
# 启用Secure Boot 与 Flash Encryption 的生产方案
### 方案1
* 由芯片端自动生成随机密钥，此时所有编译出的 images 都按正常情况烧写，芯片会在第一次 boot 时进行配置
* 通过 make menuconfig 配置 secure boot 和 flash encryption ，按照第一、二节介绍的步骤执行即可, 但这样的方式存在`以下问题`：
	* 用于签名的私钥需要保密，若泄露第三方 app 可以使用此私钥进行签名，这样就能突破 secure boot。同时用户不能遗失私钥，私钥要用于对 OTA app 签名
...
* 注意！用于签名的私钥需要保密，若泄露第三方 app 可以使用此私钥进行签名，这样就能突破 secure boot。同时用户不能遗失私钥，私钥要用于对 OTA app 签名
* 在下载工具中选择使能secure boot 和 flash encryption，重启下载工具，然后开始下载，下载工具会随机生成 secure boot key 和 flash encrypt key并将它们烧写到 efuse 中。下载工具会利用 secure boot key 生成 bootloader image digest 添加到 bootloader image中，利用 flash encrypt key 对需要加密的数据进行加密，最后将加密后的数据下载到 flash 中。下载工具会自动将 efuse 中相应的控制标记烧写为合适的值
* 下载工具是根据 security.conf 中的参数配置 secure boot 和 flash encryption:
    ...
    | `ITEM` | Function | default | secure_boot + flash_encrypt practice|
    |--|--|--|--|
    | debug_enable |  enable debug mode for security features, users can use a .pem key to generate the secure boot key and flash encryption key. This is only a debug mode in case users forget their keys.| True | False(random keys will be burnt into efuse)) |
    | debug_pem_path | file path of the .pem key|  | (leave it empty) |
    | `SECURE BOOT` | |
    | secure_boot_en | to enable secure boot function| False | True |
    | burn_secure_boot_key | to enable burning the secure boot key into efuse after downloading is finished | False | True |
    | secure_boot_force_write | to force write the secure boot key into efuse, no mater whether the efuse block has been written before. It is risky of breaking the original value | False | False |
    | secure_boot_rw_protect | to set write and read protection for secure boot key | False | True |
    | `FLASH ENCRYPTION` | |
    | flash_encryption_en | to enable flash encryption function | False | True |
    | burn_flash_encryption_key | to enable burning flash encryption key into efuse after flash downloading is finished | False | True |
    | flash_encrypt_force_write | to force write the flash encrypt key into efuse, no mater whether the efuse block has been written before. It is risky of breaking the original value | False | False |
    | flash_encrypt_rw_protect | to set write and read protection for flash encrypt key | False| True |
    | `AES KEY`| Not used yet |
    | `DISABLE FUNC` | |
    | jtag_disable | to disable JTAG access after downloading | True| True |
    |dl_encrypt_disable| to disable encrypt functions in download mode |True| True |
    |dl_decrypt_disable| to disable decrypt functions in download mode | True | True |
    |dl_cache_disable | to disable flash cache functions in download mode| True| True |

* 如果使能了 secure_boot_en or flash_encryption_en，用户必须重启下载工具